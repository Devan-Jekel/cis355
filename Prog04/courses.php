...
function printListing($apiCall) {
    ...
    if (!($obj->courses == null)) {
        ...
        if ($_GET['days']) {
            $dayModifier = $_GET['days'];
            foreach ($obj->courses as $course) {

                // if first character of days is an "M"
                if ((strcmp(substr($course->meetingTimes[0]->days, 0, 1), $dayModifier) != 0 Xor strcmp(substr($course->meetingTimes[0]->days, 1, 1), $dayModifier) != 0) || (strcmp(substr($course->meetingTimes[1]->days, 0, 1), $dayModifier) != 0 Xor strcmp(substr($course->meetingTimes[1]->days, 1, 1), $dayModifier) != 0)) {
            ...
            for ($dayCount = 0; $dayCount <= 5; $dayCount++) {
                $dayModifier = '';
                ...
                //}  OLD end of for loop NO LONGER USED COULD CAUSE PROBLEMS
                foreach ($obj->courses as $course) {
